...
func (h *Handle) HandleWrite(
	...
	if req.FileName != "" {
		...
		logutil.Infof("asdf persisted rows: %d, %v, %v", persistedCnt, req.TableName, txn.String())
		return
	}
	...
	logutil.Infof("asdf in memmory rows: %d, %v, %v", inmemCnt, req.TableName, txn.String())
	return
}
