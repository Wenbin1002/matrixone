...
func ReplayCheckpointEntry(ctx context.Context, fs fileservice.FileService, name string) (entries []*CheckpointEntry, err error) {
	reader, err := blockio.NewFileReader("", fs, name)
	if err != nil {
		return
	}
	bats, closeCB, err := reader.LoadAllColumns(ctx, nil, common.CheckpointAllocator)
